...
exports.createCustomer = async (req, res) => {
  try {
    ...
    const customerData = {
      ...
      email: email ? email.toLowerCase() : undefined,
      assignedAgent: (assignedAgent && assignedAgent !== 'none') ? assignedAgent : null,
...
exports.updateCustomer = async (req, res) => {
  try {
    ...
    if (assignedAgent !== undefined && assignedAgent !== 'none') {
      const agent = await Agent.findOne({ 
        _id: assignedAgent, 
        isActive: true 
      });
    ...
    if (assignedAgent !== undefined) {
      customer.assignedAgent = (assignedAgent && assignedAgent !== 'none') ? assignedAgent : null;
    }
    if (dateOfBirth !== undefined) customer.dateOfBirth = dateOfBirth;
    if (occupation !== undefined) customer.occupation = occupation;
    if (annualIncome !== undefined) customer.annualIncome = annualIncome;
